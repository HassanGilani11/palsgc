...
class Printec_Sticky_Module {
	...
	public function scripts(){
        ...
        wp_register_script('elementor-sticky', get_template_directory_uri() . '/assets/js/vendor/jquery.sticky.js', ['jquery'], $printec_version, true);
        wp_enqueue_script('printec-elementor-sticky', get_template_directory_uri() . '/assets/js/vendor/sticky.js', ['jquery', 'elementor-frontend', 'elementor-sticky'], $printec_version, true);
    }
    ...
    public function register_controls( Element_Base $element ) {
        $element->add_control(
            'sticky',
            [
                'label' => esc_html__( 'Sticky', 'printec' ),
        ...
        foreach ( $active_devices as $device ) {
            $label = 'desktop' === $device ? esc_html__( 'Desktop', 'printec' ) : $active_breakpoint_instances[ $device ]->get_label();
        ...
        $element->add_control(
            'sticky_on',
            [
                'label' => esc_html__( 'Sticky On', 'printec' ),
        ...
        $element->add_responsive_control(
            'sticky_offset',
            [
                'label' => esc_html__( 'Offset', 'printec' ),
        ...
        $element->add_responsive_control(
            ...
            [
                'label' => esc_html__( 'Effects Offset', 'printec' ),
                'type' => Controls_Manager::NUMBER,
                'default' => 0,
                'min' => 0,
                'max' => 1000,
        ...
        if ( $this->is_instance_of( $element, $types ) ) {
            ...
            $element->add_control(
                'sticky_parent',
                [
                    'label' => esc_html__( 'Stay In Column', 'printec' ),
