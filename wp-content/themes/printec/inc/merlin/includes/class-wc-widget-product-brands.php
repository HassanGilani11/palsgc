 ...
 * @extends WC_Widget
 */
class Printec_Widget_Product_Brands extends WC_Widget {
	...
	public function __construct() {
		...
		$this->widget_description = esc_html__('A list or dropdown of product brands.', 'printec');
		$this->widget_id          = 'woocommerce_product_brands';
		$this->widget_name        = esc_html__('Product Brands', 'printec');
		$this->settings           = array(
			'title'              => array(
				'type'  => 'text',
				'std'   => esc_html__('Product Brands', 'printec'),
				'label' => esc_html__('Title', 'printec'),
			),
			'dropdown'           => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show as dropdown', 'printec'),
			),
			'show_logo'          => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show logo brand', 'printec'),
			),
			'count'              => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Show product counts', 'printec'),
			),
			'hierarchical'       => array(
				'type'  => 'checkbox',
				'std'   => 1,
				'label' => esc_html__('Show hierarchy', 'printec'),
			),
			'show_children_only' => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Only show children of the current brand', 'printec'),
			),
			'hide_empty'         => array(
				'type'  => 'checkbox',
				'std'   => 0,
				'label' => esc_html__('Hide empty brands', 'printec'),
			),
			'max_depth'          => array(
				'type'  => 'text',
				'std'   => '',
				'label' => esc_html__('Maximum depth', 'printec'),
			),
		);
	...
	public function widget($args, $instance) {
		...
		if ($dropdown) {
			...
			$list_args['walker']                  = new Printec_Product_Brand_List_Walker;
			$list_args['title_li']                = '';
			$list_args['pad_counts']              = 1;
			$list_args['show_option_none']        = esc_html__('No product brands exist.', 'printec');
			...
			echo '<ul class="product-brands" id="printec-brands-' . $id . '">';
...
add_action('widgets_init', function () {
	register_widget('Printec_Widget_Product_Brands');
});

add_action('init', function () {
	$labels = array(
		'name'                       => esc_html__('Brands', 'printec'),
		'singular_name'              => esc_html__('Brands', 'printec'),
		'menu_name'                  => esc_html__('Brands', 'printec'),
		'all_items'                  => esc_html__('All Brands', 'printec'),
		'parent_item'                => esc_html__('Parent Brand', 'printec'),
		'parent_item_colon'          => esc_html__('Parent Brand:', 'printec'),
		'new_item_name'              => esc_html__('New Brand Name', 'printec'),
		'add_new_item'               => esc_html__('Add New Brands', 'printec'),
		'edit_item'                  => esc_html__('Edit Brand', 'printec'),
		'update_item'                => esc_html__('Update Brand', 'printec'),
		'separate_items_with_commas' => esc_html__('Separate Brand with commas', 'printec'),
		'search_items'               => esc_html__('Search Brands', 'printec'),
		'add_or_remove_items'        => esc_html__('Add or remove Brands', 'printec'),
		'choose_from_most_used'      => esc_html__('Choose from the most used Brands', 'printec'),
	);
	$args   = array(
		'labels'            => $labels,
		'hierarchical'      => true,
