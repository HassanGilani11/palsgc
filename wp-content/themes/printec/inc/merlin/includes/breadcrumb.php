...
class Printec_breadcrumb {
    ...
    private function __construct() {
        ...
        add_filter('printec_page_title', [$this, 'hide_title']);
    ...
    public function breadcrumb_register() {
        $labels = [
            'name'               => esc_html__('Breadcrumbs', 'printec'),
            'singular_name'      => esc_html__('Breadcrumb', 'printec'),
            'menu_name'          => esc_html__('Breadcrumbs', 'printec'),
            'name_admin_bar'     => esc_html__('Breadcrumb', 'printec'),
            'add_new'            => esc_html__('Add New', 'printec'),
            'add_new_item'       => esc_html__('Add New Breadcrumb', 'printec'),
            'new_item'           => esc_html__('New Header Footer & Blocks Template', 'printec'),
            'edit_item'          => esc_html__('Edit Header Footer & Blocks Template', 'printec'),
            'view_item'          => esc_html__('View Header Footer & Blocks Template', 'printec'),
            'all_items'          => esc_html__('All Breadcrumb', 'printec'),
            'search_items'       => esc_html__('Search Header Footer & Blocks Templates', 'printec'),
            'parent_item_colon'  => esc_html__('Parent Header Footer & Blocks Templates:', 'printec'),
            'not_found'          => esc_html__('No Header Footer & Blocks Templates found.', 'printec'),
            'not_found_in_trash' => esc_html__('No Header Footer & Blocks Templates found in Trash.', 'printec'),
        ];
        ...
        register_post_type('printec-breadcrumb', $args);
    }
    ...
    public function register_admin_menu() {
        add_submenu_page(
            'themes.php',
            esc_html__('Breadcrumbs', 'printec'),
            esc_html__('Breadcrumbs', 'printec'),
            'edit_pages',
            'edit.php?post_type=printec-breadcrumb'
        );
    }
    ...
    function ehf_register_metabox() {
        add_meta_box(
            'ehf-meta-box',
            esc_html__('Breadcrumb Options', 'printec'),
            [
                $this,
                'efh_metabox_render',
            ],
        ...
        <table class="hfe-options-table widefat">
            ...
            <tr class="bsf-target-rules-row hfe-options-row">
                <td class="bsf-target-rules-row-heading hfe-options-row-heading">
                    <label><?php esc_html_e('Display On', 'printec'); ?></label>
                    <i class="bsf-target-rules-heading-help dashicons dashicons-editor-help"
                       title="<?php echo esc_attr__('Add locations for where this template should appear.', 'printec'); ?>"></i>
                </td>
                <td class="bsf-target-rules-row-content hfe-options-row-content">
                    ...
                    HFE\Lib\Astra_Target_Rules_Fields::target_rule_settings_field(
                        ...
                        [
                            'title'          => esc_html__('Display Rules', 'printec'),
                            'value'          => '[{"type":"basic-global","specific":null}]',
                            'tags'           => 'site,enable,target,pages',
                            'rule_type'      => 'display',
                            'add_rule_label' => esc_html__('Add Display Rule', 'printec'),
                        ],
                        $include_locations
                    );
                    ?>
            ...
            <tr class="bsf-target-rules-row hfe-options-row">
                <td class="bsf-target-rules-row-heading hfe-options-row-heading">
                    <label><?php esc_html_e('Do Not Display On', 'printec'); ?></label>
                    <i class="bsf-target-rules-heading-help dashicons dashicons-editor-help"
                       title="<?php echo esc_attr__('Add locations for where this template should not appear.', 'printec'); ?>"></i>
                </td>
                <td class="bsf-target-rules-row-content hfe-options-row-content">
                    <?php
                    HFE\Lib\Astra_Target_Rules_Fields::target_rule_settings_field(
                        ...
                        [
                            'title'          => esc_html__('Exclude On', 'printec'),
                            'value'          => '[]',
                            'tags'           => 'site,enable,target,pages',
                            'add_rule_label' => esc_html__('Add Exclusion Rule', 'printec'),
                            'rule_type'      => 'exclude',
                        ],
                        $exclude_locations
                    );
            ...
            <tr class="bsf-target-rules-row hfe-options-row">
                <td class="bsf-target-rules-row-heading hfe-options-row-heading">
                    <label><?php esc_html_e('User Roles', 'printec'); ?></label>
                    <i class="bsf-target-rules-heading-help dashicons dashicons-editor-help" title="<?php echo esc_attr__('Display custom template based on user role.', 'printec'); ?>"></i>
                </td>
                <td class="bsf-target-rules-row-content hfe-options-row-content">
                    <?php
                    HFE\Lib\Astra_Target_Rules_Fields::target_user_role_settings_field(
                        ...
                        [
                            'title'          => esc_html__('Users', 'printec'),
                            'value'          => '[]',
                            'tags'           => 'site,enable,target,pages',
                            'add_rule_label' => esc_html__('Add User Rule', 'printec'),
                        ],
                        $users
                    );
                    ?>
    ...
    public static function get_template_id() {
        ...
        $hfe_templates = Astra_Target_Rules_Fields::get_instance()->get_posts_by_conditions('printec-breadcrumb', $option);

        foreach ($hfe_templates as $template) {
            return apply_filters('printec_breadcrumb_id', $template['id']);
        }
    ...
    public function block_template_frontend() {
        if (is_singular('printec-breadcrumb') && !current_user_can('edit_posts')) {
    ...
    function load_canvas_template($single_template) {
        ...
        if ('printec-breadcrumb' == $post->post_type) {
