...
<div class="woocommerce-pagination-wrap">
    <?php
    if ($total >= 1) {
        $pagination = printec_get_theme_option('shop_pagination', 'default');
        $pagination_text = esc_html__('Load more products','printec');
        if ('infinit' == $pagination) :
            $pagination_text = esc_html__('Loading...','printec');
            wp_enqueue_script('printec-waypoints');
        endif;
