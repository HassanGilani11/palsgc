<div id="dokan-seller-listing-wrap" class="grid-view">
    <div class="seller-listing-content">
        <?php if ($sellers['users']) : ?>
                    ...
                    <li class="dokan-single-seller woocommerce coloum-<?php echo esc_attr($per_row); ?> <?php echo (!$store_banner_id) ? 'no-banner-img' : ''; ?>">
                        <div class="store-wrapper">
                                ...
                                <div class="store-data-container">
                                    <div class="featured-favourite">
                                        <?php if ($is_store_featured) : ?>
                                            <div class="featured-label"><?php esc_html_e('Featured', 'printec'); ?></div>
                                        <?php endif ?>
                                    ...
                                    <?php if ('on' === $show_store_open_close && 'yes' === $dokan_store_time_enabled) : ?>
                                        <?php if (dokan_is_store_open($seller->ID)) { ?>
                                            <span class="dokan-store-is-open-close-status dokan-store-is-open-status" title="<?php esc_attr_e('Store is Open', 'printec'); ?>"><?php esc_html_e('Open', 'printec'); ?></span>
                                        <?php } else { ?>
                                            <span class="dokan-store-is-open-close-status dokan-store-is-closed-status" title="<?php esc_attr_e('Store is Closed', 'printec'); ?>"><?php esc_html_e('Closed', 'printec'); ?></span>
                                        <?php } ?>
                                    <?php endif ?>
                                    ...
                                    <div class="store-data <?php echo esc_attr($store_open_is_on); ?>">
                                        ...
                                        <?php if (!empty($store_rating['count'])): ?>
                                            <div class="dokan-seller-rating" title="<?php echo sprintf(esc_attr__('Rated %s out of 5', 'printec'), esc_attr($store_rating['rating'])) ?>">
                                                ...
                                                <p class="rating">
                                                    <?php echo esc_html(sprintf(__('%s out of 5', 'printec'), $store_rating['rating'])); ?>
                                                </p>
                                            </div>
                                        <?php endif ?>
                            ...
                            <div class="store-footer">
                                ...
                                <a href="<?php echo esc_url($store_url); ?>" title="<?php esc_attr_e('Visit Store', 'printec'); ?>">
            ...
            if ($num_of_pages > 1) {
                ...
                $pagination_args = array(
                    ...
                    'type'      => 'array',
                    'prev_text' => '<i class="printec-icon-angle-left"></i><span>' . __('Previous', 'printec') . '</span>',
                    'next_text' => '<span>' . __('Next', 'printec') . '</span><i class="printec-icon-angle-right"></i>',
                );
        ...
        <?php else: ?>
            <p class="dokan-error"><?php esc_html_e('No vendor found!', 'printec'); ?></p>
        <?php endif; ?>
    </div>
</div>
