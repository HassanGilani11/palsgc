...
function wcdp_add_product_single_link_customize(){
	...
	if($productCustom){
		if(isset($general['hide_addtocart']) && $general['hide_addtocart'] === 'on'){
			?>
		    <style type="text/css">
		        form.cart .quantity, form.cart .single_add_to_cart_button{ display:none}
		    </style>
		    <?php	
		}
...
function wcdp_get_button_personalize($productID){
    ...
    if(!empty($dp_publish_page)){
        ...
        $link = do_shortcode('<a href="'. $page_link .'" rel="nofollow" data-product_id="'. $productID .'" class="button product_type_customizable">'. __( "Let's Design", "wcdp" ) .'</a>');
        return $link;
	}
}
